...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		std::string tempHolderOfFile;
		int counterForFour{};

		if (file.is_open()) {
			while (!file.eof() && counterForFour < 4) {
				std::getline(file, tempHolderOfFile, '\n');
				if (tempHolderOfFile[0] != '#') {
					Book bookNew(tempHolderOfFile);
					library += bookNew;
					counterForFour++;
				}
			}
	...
	if (argc > 2) {
		...
		if (file.is_open()) {
			while (!file.eof()) {
				...
				std::getline(file, tempStoreMovies, '\n');

				if (tempStoreMovies[0] != '#' && tempStoreMovies.length() > 1) {
					Movie newMovie(tempStoreMovies);
					movies[counterMovies++] = newMovie;
				}
			}
			file.close();
		}
